...
def check_reciprocal(G, ori, curr, depth):
    # Check if depth is below or equal to zero
    if depth <= 0:
        if ori == curr:
            return True
        else:
    ...
    for node in G[curr]:
        if check_reciprocal(G, ori, node, depth - 1):
            return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    # Add reciprocal check
                    if not check_reciprocal(graph, node, page, k):
                        newrank = newrank + d * \
                            (ranks[node] / len(graph[node]))
