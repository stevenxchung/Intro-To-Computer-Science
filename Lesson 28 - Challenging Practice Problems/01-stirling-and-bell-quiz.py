...
def stirling(n, k):
    # Items is equivalent to the requested set
    if n == k:
        return 1
    # Only one requested set
    if k == 1:
        return 1
    # No requested set
    if k == 0:
        return 0
    # Requested set is greater than the number of items
    if k > n:
        return 0
    # Otherwise, do recursive operation
