...
def main_pattern(pattern, rule, ith):
    if len(pattern) == (ith + 1):
        result = pattern[ith - 1] + pattern[ith] + pattern[0]
    else:
        result = pattern[ith - 1] + pattern[ith] + pattern[ith + 1]
    # Otherwise, return outer_pattern with result and rule as input
    return outer_pattern(result, rule)


def outer_pattern(inner_pattern, rule):
    val = 0
    # Use store to determine value
    cellStore = {'...': 1, '..x': 2, '.x.': 4, '.xx': 8,
                 'x..': 16, 'x.x': 32, 'xx.': 64, 'xxx': 128}
    if cellStore[inner_pattern]:
...
def cellular_automaton(pattern, rule, depth):
    ...
    if depth == 0:
        return pattern
    # Recursively add main pattern to result
    pattern_len = len(pattern)
    for i in range(0, pattern_len):
        result += main_pattern(pattern, rule, i)
    return cellular_automaton(result, rule, depth - 1)
